  ...
  return (
    <div className="p-4 md:p-6 space-y-6 animate-enter">
      <SEO
        ...
        description="Analisis mendalam skor CSAT, tren performa mingguan, dan sebaran data dosen Cakrawala University."
      />
      {/* Header */}
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-5 leading-tight">
        <div className="flex-1 min-w-0">
          <h1
            className="font-serif-accent text-2xl md:text-3xl font-extrabold tracking-tight truncate"
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 mt-2">
            ...
            >
              {fmt(filtered.length)}{" "}
              <span className="font-normal opacity-60">Responden Valid</span>
            </p>
            {removedCount > 0 && (
            ...
            <span
              className="text-sm opacity-30 hidden sm:inline"
              style={{ color: "var(--muted)" }}
            >
              ·
            </span>
            <p
              ...
              style={{ color: "var(--muted)" }}
            >
              {fmt(dosenList.length)}{" "}
              <span className="font-normal opacity-60">Dosen</span> ·{" "}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger">
        ...
        <StatCard
          ...
          value={fmt(globalPerforma)}
          sub={scoreLabel(globalPerforma)}
          icon={TrendingUp}
          color="var(--foreground)"
        />
        <StatCard
          label="Pemahaman Materi"
          value={fmt(globalPemahaman)}
          sub={scoreLabel(globalPemahaman)}
          icon={BookOpen}
          color="var(--foreground)"
        />
        <StatCard
          label="Interaktivitas Kelas"
          value={fmt(globalInteraktif)}
          sub={scoreLabel(globalInteraktif)}
          icon={Activity}
          color="var(--foreground)"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 card p-6">
          <h2 className="section-title mb-6">
            Tren CSAT per Pertemuan (Global)
          </h2>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2.5">
            ...
            <span className="badge bg-u-navy text-brand border border-[var(--brand-border)]">
              {fmt(dosenList.length)}
            </span>
          </div>
        </div>
                    ...
                    <td>
                      <div className="flex items-center justify-start gap-2">
                        ...
                        <button
                          onClick={() =>
                            navigate(
                              `/dosen/${encodeURIComponent(d.namaDosen)}`,
                            )
                          }
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between mt-8 pt-6 border-t border-[var(--border)] gap-6">
            <div className="text-xs sm:text-sm font-medium text-[var(--muted)] text-center lg:text-left">
              ...
              <span className="text-[var(--foreground)] font-bold">
                {fmt((page - 1) * PAGE_SIZE + 1)}
              </span>{" "}
              -{" "}
              <span className="text-[var(--foreground)] font-bold">
                {fmt(Math.min(page * PAGE_SIZE, dosenList.length))}
              </span>{" "}
              dari{" "}
              <span className="text-[var(--foreground)] font-bold">
                {fmt(dosenList.length)}
                  ...
                  .map((p, i, arr) => (
                    <div key={p} className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => setPage(p)}
                        className={clsx(
                          "w-7 h-7 sm:w-8 sm:h-8 flex-shrink-0 rounded-lg text-xs font-bold transition-all border",
                          p === page
